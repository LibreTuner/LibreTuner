...
ChecksumBasic *Checksums::addBasic(uint32_t offset, uint32_t size,
                                         uint32_t target) {
    ChecksumBasic *checksum = new ChecksumBasic(offset, size, target);

    checksums_.push_back(std::unique_ptr<Checksum>(checksum));
    return checksum;
}
