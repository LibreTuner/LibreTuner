...
MazdaT1Flasher::MazdaT1Flasher(network::UdsPtr &&uds, Options&& options)
...
bool MazdaT1Flasher::do_erase() {
	...
	network::UdsResponse res = uds_->request(0xB1, eraseRequest.data(), eraseRequest.size());
	if (canceled_) {
		return false;
	}
...
bool MazdaT1Flasher::do_request_download() {
	...
	network::UdsResponse _response = uds_->request(network::UDS_REQ_REQUESTDOWNLOAD, msg.data(), msg.size());
...
bool MazdaT1Flasher::sendLoad() {
	while (left_ != 0) {
		...
		data.assign(flash_->data().begin() + sent_, flash_->data().begin() + sent_ + toSend);

		sent_ += toSend;
		left_ -= toSend;

		network::UdsResponse res = uds_->request(network::UDS_REQ_TRANSFERDATA, data.data(), data.size());
