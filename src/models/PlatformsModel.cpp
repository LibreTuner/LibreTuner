...
QModelIndex PlatformsModel::index(int row, int column, const QModelIndex & parent) const
{
    if (!parent.isValid())
    {
...
int PlatformsModel::columnCount(const QModelIndex & /*parent*/) const { return 1; }

QVariant PlatformsModel::data(const QModelIndex & index, int role) const
{
    if (platforms_ == nullptr)
        return 0;
    if (!index.isValid() || index.column() != 0 || (role != Qt::DisplayRole && role != Qt::UserRole))
    {
        return QVariant();
    }
    ...
    if (index.internalId() == 0)
    {
        ...
        const lt::Platform * platform = platforms_->at(platformIndex);
    ...
    {
        const lt::Model * model = platform->models[modelIndex].get();
        if (role == Qt::DisplayRole)
        {
...
QVariant PlatformsModel::headerData(int /*section*/, Qt::Orientation /*orientation*/, int /*role*/) const
{
    return QVariant();
}

PlatformsModel::PlatformsModel(const lt::Platforms * platforms, QObject * parent)
    : QAbstractItemModel(parent), platforms_(platforms)
{
}

Qt::ItemFlags PlatformsModel::flags(const QModelIndex & index) const { return QAbstractItemModel::flags(index); }
