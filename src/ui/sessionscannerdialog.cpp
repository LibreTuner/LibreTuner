    ...
    catchCritical(
        [this]() {
            ...
            scanner.onSuccess([this](uint8_t session_id) {
                std::stringstream ss;
                ss << "[SessionScanner] Detected 0x" << std::hex
                   << static_cast<uint32_t>(session_id);
                Logger::debug(ss.str());
                QMetaObject::invokeMethod(sessionIds_, [this, session_id]() {
                    sessionIds_->addItem(
                        QString("0x%1").arg(session_id, 0, 16));
                });
            });

            BackgroundTask<void()> task([&]() {
                scanner.scan(*uds, static_cast<uint8_t>(spinMinimum_->value()),
                             static_cast<uint8_t>(spinMaximum_->value()));
            });
