...
InterfacesWindow::InterfacesWindow(QWidget *parent)
    ...
    conn_ = InterfaceManager::get().connect(
        std::bind(&InterfacesWindow::interfacesChanged, this));
    interfacesChanged();
}
...
QModelIndex InterfacesModel::index(int row, int column,
                                   const QModelIndex &parent) const {
    if (parent.isValid()) {
        // We store the type index as the pointer
        return createIndex(row, column,
                           reinterpret_cast<void *>(parent.row() + 1));
    }
...
QModelIndex InterfacesModel::parent(const QModelIndex &child) const {
    int ind =
        static_cast<int>(reinterpret_cast<intptr_t>(child.internalPointer()));
    if (ind != 0) {
...
int InterfacesModel::columnCount(const QModelIndex &parent) const { return 1; }
