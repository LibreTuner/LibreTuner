...
enum Interaction
{
    ...
    ,
    iMultiSelect = 0x004 ///< <tt>0x004</tt> The user can select multiple
                         ///< objects by holding the modifier set by \ref
...
{
public:
    explicit QCPAbstractPaintBuffer(const QSize & size,
...
public:
    explicit QCPPaintBufferGlPbuffer(const QSize & size,
                                     double devicePixelRatio, int multisamples);
...
public:
    ...
    friend inline const QCPRange operator-(const QCPRange & range,
                                           double value);
    friend inline const QCPRange operator*(const QCPRange & range,
                                           double value);
    friend inline const QCPRange operator*(double value,
...
protected:
    ...
    virtual void
    parentPlotInitialized(QCustomPlot * parentPlot) Q_DECL_OVERRIDE;
...
public:
    ...
    */
    enum AxisType
    {
        atLeft = 0x01 ///< <tt>0x01</tt> Axis is vertical and on the left side
                      ///< of the axis rect
        ,
        atRight = 0x02 ///< <tt>0x02</tt> Axis is vertical and on the right side
                       ///< of the axis rect
        ,
    ...
    */
    ...
    Q_SLOT void
    setSelectedParts(const QCPAxis::SelectableParts & selectedParts);
...
protected:
    ...
    virtual QPointF
    getTickLabelDrawOffset(const TickLabelData & labelData) const;
...
public:
    ...
    */
    ...
    QCPScatterStyle(ScatterShape shape, const QColor & color,
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable
{
    ...
    Q_PROPERTY(QCPSelectionDecorator * selectionDecorator READ
    ...
    */
    enum PositionType
    {
        ...
        ptViewportRatio ///< Static positioning given by a fraction of the
                        ...
                        ///< left corner of the viewport/widget. setCoords(1, 1)
                        ///< will be at the bottom right corner, setCoords(0.5,
                        ///< 0) will be horizontally centered and vertically at
                        ///< the top of the viewport/widget, etc.
        ,
        ptAxisRectRatio ///< Static positioning given by a fraction of the axis
                        ...
                        ///< vertically at the top of the axis rect, etc. You
                        ///< can also go beyond the axis rect by providing
    ...
    */
    enum RefreshPriority
    {
        rpImmediateRefresh ///< Replots immediately and repaints the widget
                           ...
                           ///< the replot
        ,
        rpQueuedRefresh ///< Replots immediately, but queues the widget repaint,
                        ///< by calling QWidget::update() after the replot. This
                        ///< way multiple redundant widget repaints can be
                        ///< avoided.
        ,
        rpRefreshHint ///< Whether to use immediate or queued refresh depends on
                      ///< whether the plotting hint \ref
        ...
        rpQueuedReplot ///< Queues the entire replot for the next event loop
                       ...
                       ///< be avoided. The actual replot is then done with \ref
    ...
    void
    setBackground(const QPixmap & pm, bool scaled,
...
public:
    ...
    selectTestRect(const QRectF & rect,
        ...
        if (keyRange.contains(mainKey) &&
            valueRange.contains(
                mainValue)) // make sure data point is inside visible range, for
                            // speedup in cases where sort key isn't main key
                            // and we iterate over all points
        {
...
protected:
    ...
    QVector<QPointF>
    dataToStepLeftLines(const QVector<QCPGraphData> & data) const;
    QVector<QPointF>
    dataToStepRightLines(const QVector<QCPGraphData> & data) const;
    QVector<QPointF>
    dataToStepCenterLines(const QVector<QCPGraphData> & data) const;
    QVector<QPointF>
    dataToImpulseLines(const QVector<QCPGraphData> & data) const;
    QVector<QCPDataRange>
    getNonNanSegments(const QVector<QPointF> * lineData,
...
protected:
    ...
    void getScatters(QVector<QPointF> * scatters,
                     const QCPDataRange & dataRange, double scatterWidth) const;
...
public:
    ...
    */
    ...
    timeSeriesToOhlc(const QVector<double> & time,
...
protected:
    ...
    bool rectIntersectsLine(const QRectF & pixelRect,
