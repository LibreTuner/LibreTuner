...
public:
    ...
    Connection(const std::shared_ptr<SignalType> &signal, Func f) : signal_(signal), func_(f) { }
...
private:
    ...
    void disconnect(ConnectionType *con) {
        std::for_each(connections.begin(), connections.end(), [con](std::weak_ptr<ConnectionType> &conn) {
            if (auto sptr = conn.lock()) {
                if (sptr.get() == con) {
                    conn.reset();
                }
