...
public:
    ...
    template <typename InputIt> void write(InputIt begin, InputIt end) {
    ...
    template <typename T> void writeBigEndian(T t) {
    ...
    template <typename T> void writeLittleEndian(T t) {
        uint8_t temp[sizeof(T)];
    ...
    template <typename T> inline bool canRead() {
        return remaining() >= sizeof(T);
    }

    // Reads data and increments the pointer
    template <typename OutputIt> void read(OutputIt begin, OutputIt end) {
        std::size_t toRead = std::distance(begin, end);
        if (toRead > remaining()) {
            throw std::runtime_error(
                std::string("Not enough bytes available to read (want " +
    ...
    template <typename T> T readBigEndian() {
        if (!canRead<T>()) {
            throw std::runtime_error(
                std::string("Not enough bytes available to read ") +
                typeid(T).name());
        }
    ...
    template <typename T> T readLittleEndian() {
        if (!canRead<T>()) {
            throw std::runtime_error(
                std::string("Not enough bytes available to read ") +
                typeid(T).name());
