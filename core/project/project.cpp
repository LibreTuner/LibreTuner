...
RomPtr Project::getRom(const std::string & filename)
{
    ...
    if (!model)
        throw std::runtime_error("Unknown platform and rom combination '" +
                                 meta.platform + "' and '" +
                                 meta.model + "'");
    auto rom = std::make_shared<Calibration>(model);
...
std::vector<Calibration::MetaData> Project::queryRoms()
{
    if (!fs::exists(romsDir_))
        return std::vector<Calibration::MetaData>();
    return getMetaData<Calibration::MetaData>(romsDir_, enforceExtensions_, Calibration::extension);
}
...
{
    return generatePath(romsDir_, std::move(name), Calibration::extension);
}
...
{
    auto rom = std::make_shared<lt::Calibration>(model);
    rom->setName(name);
