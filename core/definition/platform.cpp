...
namespace nlohmann
{
template <> struct adl_serializer<lt::ChecksumPtr>
{
    static lt::ChecksumPtr from_json(const json & j)
    {
        ...
        if (auto it = j.find("modify"); it != j.end())
        {
            for (auto & section : *it)
            {
                sum->addModifiable(section.at("offset").get<std::size_t>(), section.at("size").get<std::size_t>());
            }
        }
...
void from_json(const json & j, lt::Platform & platform)
{
    ...
    if (auto auth = j.find("auth"); auth != j.end())
    {
        ...
        if (auto it = auth->find("sessionid"); it != auth->end())
        {
            ...
            platform.flashAuthOptions.session = platform.downloadAuthOptions.session;
        }
...
Model * Platform::findModel(const std::string & id) const noexcept
{
...
Model * Platform::identify(const uint8_t * data, size_t size) const noexcept
{
    for (const auto & model : models)
    {
...
std::unique_ptr<Platform> load_main(const fs::path & path)
{
    ...
    json root;
    file >> root;
    return std::make_unique<Platform>(root.get<Platform>());
}
                 ...
                 lt::Model & model) // [[expects: model.platform]]
{
    ...
    if (auto tables = j.find("tables"); tables != j.end())
    {
        ...
        {
            // Get platform table
            if (const TableDefinition * platformTable = platform.getTable(id); platformTable != nullptr)
            {
...
std::unique_ptr<Platform> Platform::loadDirectory(const std::filesystem::path & base_path)
{
    // Load main.json
    std::unique_ptr<Platform> platform = load_main(base_path / "main.json");

    // Load models
    for (auto & entry : fs::directory_iterator(base_path))
    {
        const fs::path & path = entry.path();
        if (path.extension() != ".json" || path.filename() == "main.json" || !entry.is_regular_file())
        {
            continue;
        }
        std::ifstream file(path);
        ...
        auto model = std::make_unique<Model>(*platform);
        decodeModel(j, *model);
...
Platform * Platforms::find(const std::string & id) const noexcept
{
    auto it = std::find_if(platforms_.begin(), platforms_.end(), [&](auto & p) { return p->id == id; });
...
void Platforms::loadDirectory(const std::filesystem::path & path)
{
    for (auto & entry : fs::directory_iterator(path))
    {
        ...
        {
            // Convert to unique_ptr and store
            platforms_.emplace_back(Platform::loadDirectory(entry.path()));
        }
    }
}

Platform * Platforms::first() const noexcept
{
    if (platforms_.empty())
        return nullptr;
    return platforms_.front().get();
}

Model * Platforms::find(const std::string & platformId, const std::string & modelId) const noexcept
{
