...
Table * Tune::getTable(const std::string & id, bool create)
{
    ...
    if (create)
    {
        ...
        builder.setEntries(detail::createEntries(endianness(), def->dataType, data_.view(def->offset.value(),
def->byteSize()))); builder.setBaseEntries(detail::createEntries(endianness(), def->dataType,
base_->view(def->offset.value(), def->byteSize())));
...
Calibration::MetaData Calibration::metadata() const noexcept
{
    MetaData md;
    md.name = name_;
    md.path = path_;
    ...
    {
        md.model = model_->id;
        md.platform = model_->platform_.id;
    }
    return md;
}
...
std::optional<Table> Calibration::getTable(const std::string & id, bool create)
{
    // Else, try to create a new table from base calibration
    if (create)
    {
        const TableDefinition * def = model_->getTable(id);
        if (def == nullptr)
            return {};
        ...
        builder.setEntries(
            detail::createEntries(endianness(), def->dataType, data_.view(def->offset.value(), def->byteSize())));
        builder.setBaseEntries(
            detail::createEntries(endianness(), def->dataType, view(def->offset.value(), def->byteSize())));
...
std::unique_ptr<Axis> Calibration::getAxis(const std::string & id, bool create)
{
    // Check axes cache
    if (!create)
        return nullptr;

    const lt::Platform & platform = model_->platform();
    ...
    std::visit(
        [&](auto && typeDefinition) {
            ...
            else if constexpr (std::is_same_v<T, MemoryAxisDefinition>)
            {
                // Memory axis
                // TODO: offsets should work the same as tables.
                int offset = model_->getAxisOffset(id);
                int size = typeDefinition.size * static_cast<int>(dataTypeSize(def->dataType));
                if (offset + size > data_.size())
                {
                    throw std::runtime_error("axis exceeds rom size (rom size: " + std::to_string(data_.size()) +
                                             ", axis ends at " + std::to_string(offset + size) + ")");
                }
...
// CEREAL_CLASS_VERSION(lt::Calibration::MetaData, 1)